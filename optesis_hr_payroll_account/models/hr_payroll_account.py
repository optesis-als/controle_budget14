...
class HrPayslipLine(models.Model):
    ...
    def _get_partner_id(self, credit_account):
        """
    Obtenir le partner_id de la ligne de payslip à utiliser dans account_move_line
        """
...
class HrPayslip(models.Model):
    ...
    date = fields.Date('Date Account', states={'draft': [('readonly', False)]}, readonly=True,
        help="Keep empty to use the period of the validation(Payslip) date.", invisible="0")
    journal_id = fields.Many2one('account.journal', 'Salary Journal', readonly=True, required=True,
        states={'draft': [('readonly', False)]}, default=lambda self: self.env['account.journal'].search([('type', '=', 'general')], limit=1), invisible="0")
    move_id = fields.Many2one('account.move', 'Accounting Entry', readonly=True, copy=False, invisible="0")
    ...
    #on comptabilise les fiche de paie
    ...
    def pay_payslip(self):
        ...
        for slip in self:
            ...
            slip.write({'move_id': move.id, 'date': date, 'state': 'compta'})
            move.post()
