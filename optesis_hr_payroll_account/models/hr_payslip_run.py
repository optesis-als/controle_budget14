...
class HrPayslipRunExtend(models.Model):
    ...
    #pour comptabliser l'enregistrement sur le model account.move et cloture les bulletin
    def pay_payslip(self):
        ...
        for slip_obj in self.slip_ids:
            if slip_obj.state != 'compta':
                 #commenté par diw car y'a des regles qui ne sont pas encore cree
                ...
                slip_obj.write({'move_id': move.id, 'date': date, 'state': 'compta'})
        self.write({'state': 'compta'})