...
class PayslipRule(models.Model):
    ...
    def compute_sheet(self):
        for payslip in self:
            ...
            payslip.get_trimf_recalculer()
            payslip.update_recompute_trimf()
            #payslip.get_annual_trimf()
                    ...
                    if not ((bonus.date_to < record.date_from or bonus.date_from > record.date_to)
                        ...
                        if bonus.salary_rule_id.is_prorata:
                            bonus.salary_rule_id.write({
                                'amount_fix': round(bonus.amount * (worked_days.WORK100.number_of_days) / payslip.number_of_days_work), })
                        else:
            ...
            for contract in contracts:
                ...
                for rule in sorted_rules:
                    ...
                    if rule._satisfy_condition(localdict) and rule.id not in blacklist:
                        ...
                        if rule.code == 'C7020': # cumul ir
                            amount = payslip.get_cumul_ir(ir_of_current_payslip)
                            
                        if rule.code == 'C7060': # ir annuel
                            amount = payslip.get_ir_annuel(brut_imposable_of_current_payslip)
                            
                        if rule.code == 'C7570': #trimf annual
                            amount = payslip.get_annual_trimf(brut_imposable_of_current_payslip)
                        
                        if rule.code == 'C2030':  # indemnite de fin de contrat
                            amount = payslip.compute_brut_salaire_of_current_payslip( brut_salaire_of_current_payslip)
                            
                        if rule.code == 'C5520': #provision retraite
                            amount = payslip.compute_provision_retraite(brut_salaire_of_current_payslip)
                        
                        if rule.code == 'C3030':  # indemnite de licenciement
                            amount = payslip.compute_indemnite_licenciement(brut_salaire_of_current_payslip)
                        ...
                        if rule.code == 'C3040':  # indemnite de deces 
                            amount = payslip.compute_indemnite_deces(retraire_of_current_payslip)  
                    ...
                    if payslip.date_from.year == payslip.date_part_ir.year:          
                        if payslip.date_from.month == payslip.date_part_ir.month:
                            ...
                            cumul_tranche_ipm = sum(
                                        payslip_line.total for payslip_line in payslip.line_ids if payslip_line.code == "C6015")#Impot progressif
            ...
            for line in self.env['hr.payslip'].search([('employee_id', '=', payslip.employee_id.id)]):
                if datetime.strptime(str(line.date_from), server_dt).year == year:
                    ...
                    cumul_ir_recal += sum(payslip_line.total for payslip_line in line.line_ids if
                            payslip_line.code == "C7030")#ir recal =
                    ...
                    for payslip_line in self.line_ids:
                        if payslip_line.code == "C7010":
                            ir_provisoire = payslip_line.amount 
                                    
                    for payslip_line in self.line_ids:
                        if payslip_line.code == "C7050":
                            ir_regul_current = payslip_line.amount
                            
                    [obj.write({'amount': round(cumul_ir_recalcul)}) for obj in
                                 payslip.line_ids if obj.code == "C7040"]#ir recalculer cumule
                    

                    [obj.write({'amount': round(ir_provisoire - ir_regul_current)}) for obj in
