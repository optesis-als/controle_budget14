...
class ProvisionRetraiteRuleInput(models.Model):
    ...
    def compute_provision_retraite(self, brut_of_current_payslip):
        for payslip in self:
            ...
            if len(payslip_ids) >= 12:#si il fait 1ans ds l'entreprise
                cumul_brut = 0
                for line in payslip_ids_precedent:
    ...
    def indemnite_deces(self, brut_of_current_payslip):
        for payslip in self:
            payslip_ids = self.env['hr.payslip'].search([('employee_id', '=', payslip.employee_id.id)], order='id desc',limit=12)
            payslip_ids_precedent = self.env['hr.payslip'].search([('employee_id', '=', payslip.employee_id.id)], order='id desc',limit=11)
            if len(payslip_ids) >= 12:#si il fait 1ans ds l'entreprise
                ...
                diff = relativedelta.relativedelta(payslip.contract_id.dateAnciennete, payslip.date_to)
                if diff.years <= 5:
                    indemnite_deces_calcul = self.compute_dece_moin_cinq(moy_brut, -diff.years, -diff.months,
                                                                   -diff.days)  # moy_brut*(dur.days/360)*0.25 or dur=diff=payslip.date_to - payslip.contract_id.dateAnciennete

                elif 5 < diff.years <= 10:
                    indemnite_deces_calcul = self.compute_dece_moin_cinq(moy_brut, 5, 0, 0)
                    indemnite_deces_calcul += self.compute_dece_plus_cinq(moy_brut, -diff.years - 5, -diff.months,
                                                                    -diff.days)  # moy_brut*(dur.days/360)*0.3

                else:
                    indemnite_deces_calcul = self.compute_dece_moin_cinq(moy_brut, 5, 0, 0)
                    indemnite_deces_calcul += self.compute_dece_plus_cinq(moy_brut, -diff.years - 5, 0, 0)
                    indemnite_deces_calcul += self.compute_dece_plus_dix(moy_brut, -diff.years - 10, -diff.months, -diff.days)

                return round(indemnite_deces_calcul)
            return 0.0

    def compute_dece_moin_cinq(self, moyb, years, months, days):
        amount_for_year = (moyb * 0.25) * float(years)
        amount_for_month = moyb * 0.25 * (float(months) / 12)
        amount_for_days = moyb * 0.25 * (float(days) / 365)
        return round(amount_for_year + amount_for_month + amount_for_days)

    def compute_dece_plus_cinq(self, moyb, years, months, days):
        amount_for_year = (moyb * 0.3) * float(years)
        amount_for_month = moyb * 0.3 * (float(months) / 12)
        amount_for_days = moyb * 0.3 * (float(days) / 365)
        return round(amount_for_year + amount_for_month + amount_for_days)

    def compute_dece_plus_dix(self, moyb, years, months, days):
        amount_for_year = (moyb * 0.4) * float(years)
        amount_for_month = moyb * 0.4 * (float(months) / 12)
        amount_for_days = moyb * 0.4 * (float(days) / 365)
        return round(amount_for_year + amount_for_month + amount_for_days)
    ...
    def compute_indemnite_deces(self, brut_of_current_payslip):
        for payslip in self:
            if payslip.contract_id.motif:
                return self.indemnite_deces(brut_of_current_payslip)


            
     
    def indemnite_licenciement(self, brut_of_current_payslip):
        for payslip in self:
            payslip_ids = self.env['hr.payslip'].search([('employee_id', '=', payslip.employee_id.id)], order='id desc',limit=12)
            payslip_ids_precedent = self.env['hr.payslip'].search([('employee_id', '=', payslip.employee_id.id)], order='id desc',limit=11)
            if len(payslip_ids) >= 12:#si il fait 1ans ds l'entreprise
    ...
    def compute_indemnite_licenciement(self, brut_of_current_payslip):
        for payslip in self:
            if payslip.contract_id.motif:
                return self.indemnite_licenciement(brut_of_current_payslip)
    ...
    def compute_depart_retraite(self, retraire_of_current_payslip):
        for payslip in self:
            ...
            cumul_retraite = 0.0
            for line in payslip_ids:
                cumul_retraite += sum(payslip_line.total for payslip_line in line.line_ids if payslip_line.code == "C5520")  
